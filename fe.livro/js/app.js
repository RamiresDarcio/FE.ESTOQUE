...
async function loadBookDetail() {
    ...
    try {
        ...
        document.querySelector('[data-detail]').innerHTML = `<img class="detail-image" src="${imageOrPlaceholder(book.imagem)}" alt="Capa de ${escapeHtml(book.titulo)}"><div><span class="eyebrow">${escapeHtml(book.genero)}</span><h1>${escapeHtml(book.titulo)}</h1><p class="lead">${escapeHtml(book.resumo)}</p><dl class="details"><div><dt>Autor</dt><dd>${escapeHtml(book.autor)}</dd></div><div><dt>Editora</dt><dd>${escapeHtml(book.editora)}</dd></div><div><dt>Ano</dt><dd>${book.anoPublicacao}</dd></div><div><dt>Estoque</dt><dd data-stock-value>${book.quantidade} unidades</dd></div><div><dt>Preço</dt><dd>${money(book.preco)}</dd></div></dl><form class="inline-fields" data-stock-form><input type="number" name="quantidade" min="0" value="${book.quantidade}" aria-label="Nova quantidade em estoque"><button class="button secondary" type="submit">Atualizar estoque</button></form><div class="card-actions"><a class="button primary" href="gerenciar_de_produto.html?id=${book.id}">Editar livro</a><button class="button danger" data-detail-delete>Excluir</button></div></div>`;
        document.querySelector('[data-stock-form]').addEventListener('submit', async event => { event.preventDefault(); const quantidade = Number(new FormData(event.currentTarget).get('quantidade')); try { const updated = await apiRequest(`/livros/${id}/estoque`, { method: 'PATCH', body: JSON.stringify({ quantidade }) }); document.querySelector('[data-stock-value]').textContent = `${updated.quantidade} unidades`; showMessage('Estoque atualizado com sucesso.'); } catch (error) { showMessage(error.message, 'error'); } });
        document.querySelector('[data-detail-delete]').addEventListener('click', async () => { if (!confirm('Tem certeza que deseja excluir este livro?')) return; try { await apiRequest(`/livros/${id}`, { method: 'DELETE' }); location.href = 'pesquisar.html'; } catch (error) { showMessage(error.message, 'error'); } });
    } catch (error) { showMessage(error.message, 'error'); }
}
